...
function regProd(){
  ...
  //alert($("#register").serialize());
  //alert(sendData);
  $.post( "http://localhost:3000/registroProduto", sendData )
    .done(function(data){
...
function regPet(){
  ...
  //alert($("#register").serialize());
  //alert(sendData);
  $.post( "http://localhost:3000/registroPet", sendData )
    .done(function(data){
...
function logIn(){
  ...
  //alert($("#register").serialize());
  //alert(sendData);
  $.post("http://localhost:3000/login", sendData )
    .done(function(data){
    if(data.search("Erro") < 0 ){
...
function loadheader(){
  //alert("a");
  $.post( "http://localhost:3000/header", "admin=" + usuarioLogado.UserAdmin + "&nome=" + usuarioLogado.UserName )
    .done(function(data){
...
function loadSobreNos(){
  //alert(numVendas);
  $.get( "http://localhost:3000/SobreNos" )
    .done(function(data){
    document.getElementById("success").innerHTML = data;
  });
}
...
function loadShop(animal, tipo){
  //alert("loadShop");
  $.post( "http://localhost:3000/loja",  "animal=" + animal + "&tipo=" + tipo)
    .done(function(data){
    document.getElementById("success").innerHTML = data;
  });
  w3_open();
...
function loadProdDescription(id){
  ...
  if(usuarioLogado == null){
    $.post( "http://localhost:3000/descricao", "id=" + id + "&eAdmin=0&numVendas=" + numVendas )
      .done(function(data){
      //alert(data);
      document.getElementById("success").innerHTML = data;
    });
    ...
    $.post( "http://localhost:3000/descricao", "id=" + id + "&eAdmin=" + usuarioLogado.UserAdmin+"&numVendas=" + numVendas )
      .done(function(data){
      document.getElementById("success").innerHTML = data;
    });
  }
...
function realizarVenda(){
  ...
  for(i = 0 ; i < compras.produtos.length ; i++){
    ...
    var produto = {
      "produtoID": compras.produtos[i].produtoID,
      "produtoID": compras.produtos[i].produtoID,
      "foto": compras.produtos[i].foto,
      "desc": compras.produtos[i].desc,
      "preco": parseInt(compras.produtos[i].preco),
      "qtd" : parseInt(qtd)
    }
    vendasJSON.push(produto);
    //if (i < compras.produtos.length-1) vendasJSON += ',';
  }
  //vendasJSON += ']}';

  $.post( "http://localhost:3000/registroVenda", "email=" + usuarioLogado.UserEmail + "&produtos=" + JSON.stringify(vendasJSON)) //registra produtos vendidos
    .done(function(data){
    vendaID = data;
  });

  alert(JSON.stringify(vendasJSON));
...
function loadMinhaConta(){
  var page;
  $.post( "http://localhost:3000/minhaConta", "email=" + usuarioLogado.UserEmail)
    .done(function(data){
    page = data;
  });
  //alert("pegar pet");
  $.post( "http://localhost:3000/meusPets", "email=" + usuarioLogado.UserEmail)
    .done(function(data){
    page+=data;
    document.getElementById("success").innerHTML = page;
  });
...
function regAdmin(){
  ...
  $.post("http://localhost:3000/registroAdmin", sendData + "&foto=" + foto)
    .done(function(data){
